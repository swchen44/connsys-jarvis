...
若 Expert 的 `expert.json` `is_base=false` 且無 `is_base=true` 的 dependency：
...
@connsys-jarvis/wifi-bora/wifi-bora-base-expert/soul.md
...

@CLAUDE.local.md
```

> `@include` 直接指向 expert 資料夾內的檔案（symlink 到 `connsys-jarvis/`），更新 expert 後不需重新安裝即可生效。soul.md 和 expert.md 均為可選，若檔案不存在則跳過。

### 9.2 多 Expert 安裝（--add 疊加）

安裝多個 Expert 時，CLAUDE.md 包含 identity 的 soul.md + expert.md（可選），加上所有 Base Experts 的 soul.md + expert.md（可選）（`## Base Experts` 區段）：
...
@connsys-jarvis/framework/framework-base-expert/expert.md
@connsys-jarvis/wifi-bora/wifi-bora-base-expert/soul.md
@connsys-jarvis/wifi-bora/wifi-bora-base-expert/expert.md

@CLAUDE.local.md
```

### 9.3 Base Expert 特殊規則（is_base=true）

**規則**：任何 `expert.json` 中 `is_base=true` 的 Expert（不論是否為 identity），其 soul.md + expert.md（可選）寫入 CLAUDE.md 的 `## Base Experts` 區段。

**偵測範圍**：遍歷依賴樹
- 從每個已安裝 Expert 出發
- 讀取 `expert.json.dependencies`
- 凡路徑上的 Expert 有 `is_base=true`，即納入 Base Experts 區段
- `visited` set 防止 diamond dependency 重複輸出

**範例**：
```
A（identity）→ B（is_base=true）→ C（is_base=true）→ D（is_base=false）
```
CLAUDE.md 包含：A 的 soul.md + expert.md（主區段） + B 的 soul.md + expert.md（Base Experts）+ C 的 soul.md + expert.md（Base Experts）
D 不輸出（is_base=false）
