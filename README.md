...
```bash
# 安裝 WiFi Bora Base Expert（含 sys-bora-base-expert dependency）
bash connsys-jarvis/wifi-bora/wifi-bora-base-expert/install_claudecode.sh
...
> **可同時安裝多個 Expert** — 共用的 dependency（如 sys-bora-base-expert）不會重複安裝。

或用 curl 直接從 GitHub 執行（不需先 clone repo）：

```bash
curl -sL https://raw.githubusercontent.com/swchen44/connsys-jarvis/main/wifi-bora/wifi-bora-base-expert/install_claudecode.sh | bash
```
