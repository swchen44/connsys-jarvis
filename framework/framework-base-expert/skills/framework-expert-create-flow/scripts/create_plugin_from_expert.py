...
Design:
  ...
  - marketplace.json uses relative paths from repo root to each expert directory,
    and declares direct dependencies so Claude Code auto-resolves transitive deps

Note (verified 2026-04):
  Claude Code auto-installs plugin dependencies when marketplace.json entries
  declare a `dependencies` field. No manual dependency ordering needed.
